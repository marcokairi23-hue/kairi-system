...
- **חומרה:** משבש
- **סטטוס:** ❌ נסגר כלא-בעיה (11.08.2026, הכרעת מרקו) — כל הזמנה כבר עצמאית לחלוטין מ-`customers` בזמן ריצה: `customer_name_snapshot`/`phone_snapshot` על `orders`, ובנוסף ה-PDF הקפוא שנוצר במעבר ממתין-לגבייה→הבא (§4.6), הם "מקור האמת" לתצוגה/הדפסה — לא שאילתה חיה לטבלת `customers`. כפילות שם/רשומה בטבלת `customers` לא משפיעה על שום תהליך או תצוגה בפועל. לא ייחסם/יתוקן.
...
- **חומרה:** משבש
- **סטטוס:** ✅ תוקן (11.08.2026, במסגרת מעבר יחידות המטר §4.1) — `NewOrder.tsx`/`EditOrder.tsx` עברו מ-`.filter(Boolean)` ל-`.filter(h => !isNaN(h))`, כך שערך 0 לגיטימי נשמר ורק קלט לא-מספרי (NaN) מסונן. נבדק E2E בפועל.
- **ראיה:** ב-DB יש 6 שורות ב-`order_items` עם `heights_cm = '{}'` (מערך ריק) ולעיתים גם `width_cm = 0`, למשל item `194bb6b9...` בהזמנה `7236fc82...` (מספר 9022, שכבר עברה לסטטוס "הושלם"). שורות אלה נותרות כפי שהן (סרק היסטורי) — התיקון מונע הישנות מעתה והלאה.
...
### 20. מסך "פריטים" מציג פריטים של הזמנות שעדיין לא פעילות (הצעת מחיר / ממתין לגבייה)
- **התנהגות בפועל:** `ItemsList.tsx` (מסך `/items`) שולף את כל שורות `order_items` ללא כל סינון לפי סטטוס ההזמנה של האב. פריט שנוצר בהזמנה שעדיין `quote` או `pending_payment` — כלומר עוד לא אושרה/נגבתה ולא נכנסה לביצוע בפועל — מופיע בדיוק כמו פריט של הזמנה שכבר `in_production`, כולל אפשרות לבחור אותו, לקדם סטטוס, להדפיס הוראות עבודה וכו'.
- **מיקום:** `src/pages/items/ItemsList.tsx` פונקציית `load()` — `supabase.from('order_items').select(...)` בלי `.filter`/`.in` על `orders.status`.
- **התנהגות מצופה:** פריט מופיע במסך רק אם ההזמנה שלו "פעילה" — יצאה משלב המכירה/הגבייה. זוהה ותועד ע"י מרקו.
- **חומרה:** משבש
- **סטטוס:** ✅ תוקן (11.08.2026) — נוסף קבוע משותף `ACTIVE_ORDER_STATUSES` (`ready`, `in_production`, `ready_for_install`, `picked_by_installer`, `completed`) ב-`statusHelpers.ts`, ו-`ItemsList.tsx` מסנן לפיו לפני התצוגה. `draft`/`quote`/`pending_payment` מוסתרים תמיד; `cancelled` מוסתר גם הוא באותה רשימה (וממילא לפי SPEC §2.2 ביטול חסום אחרי כניסה לייצור, כך שלא אמור להיות מצב של הזמנה מבוטלת עם פריטים בעבודה).
- **ראיה:** `grep -n "from('order_items')" src/pages/items/ItemsList.tsx` — קריאה בלי שום תנאי סטטוס לפני התיקון.

---

## סיכום

**סה"כ ליקויים שתועדו: 20** (#20 נוסף 11.08.2026 — זוהה ע"י מרקו במהלך שימוש)

### פירוק לפי חומרה
| חומרה | כמות | מספרים |
|---|---|---|
| חוסם | 1 | #5 |
| משבש | 9 | #1, #2, #3, #4, #6, #7, #13(פוטנציאלי)*, #15, #20 |
...
**סטטוס תיקון בפועל (11.08.2026):** ✅ תוקנו #5, #15, #3, #20. ❌ #1 נסגר כלא-בעיה. פתוחים מקבוצת "משבש": #2, #4, #6, #7.
