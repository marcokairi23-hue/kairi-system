...
> **הוראות שימוש:** הדבק את המסמך הזה בתחילת שיחה חדשה עם כל כלי AI. זו נקודת הכניסה — היא מפנה למסמכי docs/ הנוספים (ראו "סדר קריאה" למטה) שמכילים את הפירוט המלא.

## מצב עבודה נוכחי (31.08.2026)

**עמוד Orders — כרטיסיות במקום טאבים + אייקונים במקום אימוג'י + בחינת "קדם" מול "פריטים" (30-31.08.2026). ממתין לאישור ידני של מרקו בדפדפן.**

מה נעשה (הכל ב-`src/pages/orders/`, פרט להתקנת `lucide-react`):
- **מבנה כרטיסיות חדש ב-`OrdersList.tsx`** — הטאבים הישנים פורקו לשתי רמות: 3 כרטיסיות ראשיות (כל ההזמנות / יתרה פתוחה / הצעות מחיר, `grid grid-cols-3`), ומתחתיהן כרטיסיות-משנה "לפי סטטוס" (ממתין לגבייה/חדש לביצוע/בייצור/מוכן/הושלמו) מוזחות עם `border-r-2` (אותו דפוס ויזואלי שכבר קיים ב-`ScreenManager` לנטינג מסך/קומפוננטה) — כדי שיהיה ברור שהן כפופות. שתי הרמות מפעילות את אותו מנגנון `?tab=` כמו קודם; לוגיקת הסינון לא השתנתה מהותית. כרטיסיות-המשנה הוגדלו (מ-pills קטנים ל-`card` אמיתיות בגריד) לפי בקשת מרקו.
- **אימוג'ים → lucide-react** — הותקנה הספרייה. הוחלפו: 7 האייקונים ב-`OrderActions.tsx` (הדפס/עבודה/WhatsApp/עריכה/תשלום/פריטים/קדם — כולם `text-brand`), `⚠️` ב-`OrderCardView.tsx` (ל-`TriangleAlert`), כפתורי כרטיסיות/רשימה וחצי המיון ב-`OrdersList.tsx`/`OrderTableView.tsx`. אימוג'י בתוך **טקסט הודעת WhatsApp ללקוח** (`OrderActions.tsx`, `doWhatsApp`) לא נגעו בהם בכוונה — זה תוכן הודעה ללקוח, לא אייקון UI.
- **"קדם" מול "פריטים" — בחינה מכוונת, לא הכרעה סופית.** מרקו זיהה חפיפה בין שני הכפתורים ורצה לבחון שימוש בפועל לפני שמחליטים על גורל "קדם":
  - כיוון החץ של "קדם" תוקן (ArrowLeft, בהתאם לזרימת RTL — היה הפוך).
  - `ItemStatusDialog.tsx` (נפתח מכפתור "פריטים") קיבל כפתור חדש **"הדפסת דוח עבודה"** על הפריטים המסומנים — קורא ל-`printWorkOrder` הקיים (`src/pages/items/printWork.ts`, אותו מסמך שנפתח היום ממסך "פריטים"), לא נבנה מסמך/רכיב חדש.
  - תנאי ההצגה של כפתור "פריטים" הורחב מ-`ACTIVE_ORDER_STATUSES.includes(order.status)` לזהה בדיוק לתנאי של "קדם" (`ORDER_STATUS_NEXT[order.status]` קיים) — שני הכפתורים מופיעים עכשיו יחד באותם 6 סטטוסים (`quote`→`picked_by_installer`), כולל שני הסטטוסים ש"פריטים" לא הופיע בהם קודם (`quote`, `pending_payment`).
- **מחוץ להיקף בכוונה:** `OrderDetail.tsx`, `NewOrder.tsx`, `EditOrder.tsx`, `printOrder.ts`, `ShareButton.tsx`, `SyncOrderDialog.tsx` — לא הזמנות-כרשימה/כרטיסיות, לא נדונו.

בדיקת קריטריון קבלה: `npm run build` עבר נקי אחרי כל שלב. **טרם אומת ידנית בדפדפן ע"י מרקו** — לבדוק ב-`/orders`: 3 הכרטיסיות + כרטיסיות-המשנה, כל כפתורי הפעולה (כולל הדפסת דוח עבודה מתוך "פריטים"), והופעת "פריטים" גם ב-quote/pending_payment.

---

## מצב קודם (30.08.2026)

**תיקון: אכיפת feature flags שהיו חסרות (נמצא ואומת ע"י מרקו, 30.08.2026).**
אחרי שספרינטים A-C (פירוט למטה) סיימו את תשתית ה-feature flags, נמצאו בפועל שני מקומות שבהם ה-UI לא צרך את הדגלים למרות שה-DB/מסך הניהול תמכו בהם:
- `src/components/Layout.tsx` — פריטי הנאב-בר "משתמשים"/"הגדרות" הוצגו תמיד לאדמין ללא תלות בדגל (`featureKey` לא הוגדר להם), בעוד ה-routes המתאימים כן נאכפו (ממיגרציה 0013). תוקן: נוסף `featureKey: 'users'`/`'settings'` + `useFeature` תואם ב-`featureVisible`.
- `src/pages/Dashboard.tsx` — 7 הקומפוננטות שהוגדרו תחת `dashboard` ב-seed (`balance`, `statusCards`, `newOrderBtn`, `stuck`, `inProduction`, `recentActivity`, `shortcuts`) — נראו במסך ניהול המסכים, אבל אף אחת לא נקראה בפועל ב-Dashboard.tsx. כיבוי "הזמנות תקועות"/"בייצור עכשיו" וכו' לא השפיע כלל על מה שמוצג. תוקן: נוספו 7 קריאות `useFeature` (בסדר קבוע, לפי rules-of-hooks) ועטיפת כל בלוק ברינדור מותנה.

בדיקת קריטריון קבלה: `npm run build` עבר נקי אחרי כל אחד משני התיקונים. **אומת ידנית בדפדפן ע"י מרקו — עובד כמצופה** (הדלקה/כיבוי בסקרין-מנג'ר מעדכן מיד את הנאב-בר ואת הדשבורד).

---

**פרויקט Feature Flags הושלם ואומת ידנית ע"י מרקו — עובד כמצופה. אין עבודה פתוחה על הפרויקט הזה.**
טבלאות + RLS + seed (11 מסכים, 8 קומפוננטות, 76 הרשאות), `useFeature`/`FeatureFlagsProvider`, אכיפה מלאה ב-navbar וב-routes, ומסך ניהול חי ב-`/screen-manager` (admin בלבד) לשליטה בהדלקה/כיבוי + הרשאות תפקיד בלי לגעת ב-DB ידנית. פירוט מלא + כל ההחלטות/סטיות שאושרו בדרך: ראו "יומן ספרינטים — Feature Flags" למטה (מהחדש לישן: C ← 0013 ← B2 ← B1 ← A). הכל דחוף ל-`origin/main` (עד commit `3c50497`) ופרוס אוטומטית ל-Cloudflare Pages.

**היחיד שעדיין פתוח בפרויקט כולו — לא קשור ל-feature flags:** הניסוי העיצובי ב-`ProductionBoard.tsx` (ראו "מצב קודם" למטה) עדיין בעצירה מכוונת של מרקו, ממתין להנחיות עיצוב חדשות שיחולו על כלל המערכת. **אל תניח כלום ואל תמשיך בו בלי לשאול קודם אם הגיעו הנחיות חדשות.**

אין כרגע שום משימה אחרת פעילה או ממתינה לאישור. תחילת session חדש = לקרוא את הסעיף הזה, ואז לשאול את מרקו מה הבא.

## יומן ספרינטים — Feature Flags (ארכיון, החדש למעלה)
...
- **בדיקה ידנית בדפדפן בוצעה ע"י מרקו (24.08.2026) — הכל עבר תקין**, כולל עדכון navbar חי (בלי רענון דף) אחרי הזזת סוויצ' items.

**זו נקודת הסיום של פרויקט ה-feature flags** (ספרינטים A, B1, B2, מיגרציה 0013, C). המנגנון מלא: תשתית DB, אכיפה ב-navbar+routes, ו-UI ניהול חי. **אומת ידנית ע"י מרקו — עובד כמצופה.**
...
## מצב קודם (23.08.2026, עדיין רלוונטי — הפריט הפתוח היחיד)

**בעצירה, ממתין להנחיות.** נבנה `src/pages/items/ProductionBoard.tsx` (route `/production`, קישור ב-nav "לוח ייצור - בדיקה") כניסוי עיצובי לפי פרויקט ה-Design "Curtains order system redesign". מרקו עצר את העבודה במכוון: הכיוון העיצובי הרחב עומד להתהפך — במקום redesign, חזרה לרפליקה קרובה של המערכת הקיימת עם שדרוגי workflow מינוריים בלבד. פרטים מלאים: `SPEC.md` §4.10, `WORKPLAN.md` §8 (רשומה 23.08.2026). לפני שממשיכים — לבדוק אם הגיעו הנחיות עיצוב חדשות וגורפות; אם לא, לא להניח כלום ולשאול. (השלב הזה כן קיבל commit בסופו של דבר — `44a1b14` — למרות שברשומה המקורית נכתב שלא; ראו הערה מתוקנת ב-`WORKPLAN.md` §8.)
...
1. **[BUSINESS.md](BUSINESS.md)** — איך העסק מתנהל היום, תפקידי הצוות, שרשרת ההזמנה.
2. **[SPEC.md](SPEC.md)** — אפיון המערכת מא' עד ת', מאושר סעיף-סעיף.
3. **[WORKPLAN.md](WORKPLAN.md)** — כללי עבודה, יומן שינויים, מצב נוכחי, רשימת פתוחים.
4. **[PERMISSIONS_MAP.md](PERMISSIONS_MAP.md)** — מפת הרשאות read-only, אודיט ישיר מול ה-DB (roles, RLS, פערי UI/RLS).
5. **[IDEAS.md](IDEAS.md)** — רעיונות עתידיים, לא מאושרים.

(עודכן 24.08.2026 — הרשימה הזו הפנתה בעבר ל-`PROJECT.md`/`ARCHITECTURE.md` שלא קיימים בפועל ב-`docs/`; תוקן לרשימת הקבצים האמיתית.)
